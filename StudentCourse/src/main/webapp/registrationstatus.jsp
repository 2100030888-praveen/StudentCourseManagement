    ...
    <tbody>
      ...
      <c:forEach items="${registrationList}" var="registration">
            <tr>
                ...
                <td>${registration.sem}</td>
                <c:if test="${registration.status==true}">
					<td bgcolor="green">ACTIVE</td>
					</c:if>
					<c:if test="${registration.status==false}">
					<td bgcolor="red">INACTIVE</td>
					</c:if>
					<td>
					<a href='<c:url value="setstatus?id=${registration.id}&status=true"></c:url>'>Active</a>
					<a href='<c:url value="setstatus?id=${registration.id}&status=false"></c:url>'>Inactive</a>
					</td>
...
<script>
  $(document).ready(function() {
   ...
   <script>
function confirmEdit(studentId, departmentId) {
    var confirmation = confirm("Are you sure you want to edit this student?");
    
    if (confirmation) {
        window.location.href = "editStudent?id=" + studentId + "&sdeptid=" + departmentId;
    }
}
